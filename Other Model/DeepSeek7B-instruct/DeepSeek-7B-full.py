# deepseek_full_ft.py
# ==================== DEEPSEEK 7B INSTRUCT - FULL FINETUNE (1x GPU, NO LoRA/NO QUANT) ====================
...
# ==================== PARAMS ====================
MODEL_ID    = "deepseek-ai/deepseek-llm-7b-chat"  # ✅ Real open-source DeepSeek!
MODEL_SHORT = "deepseek_llm_7B"
...
SAVE_STEPS   = int(os.getenv("SAVE_STEPS", 150))      # 150 (au lieu de 200) - plus fréquent pour tracking
EVAL_STEPS   = int(os.getenv("EVAL_STEPS", 150))      # 150 - évaluer plus souvent
LOG_STEPS    = int(os.getenv("LOG_STEPS", 25))        # 25 (au lieu de 50) - logs plus fréquents
...
MODEL_DIR    = Path(__file__).parent / "model"  # Sauvegarde dans Other-instruct/model
...
def sft_config():
    ...
    return SFTConfig(
        ...
        max_grad_norm=1.0,
        eval_strategy="steps",     # Évaluer pendant l'entraînement
        eval_steps=EVAL_STEPS,     # Tous les 150 steps
        logging_strategy="steps",
        logging_steps=LOG_STEPS,   # Tous les 25 steps
        save_strategy="steps",
        save_steps=SAVE_STEPS,     # Tous les 150 steps (= même que eval_steps)
        save_total_limit=2,        # Garder max 2 checkpoints (au lieu de 1) pour best model tracking
        load_best_model_at_end=True,    # Charger le meilleur modèle (eval_loss minimum)
...
def main():
    ...
    print("\n🔮 Generating predictions for 100 evaluation samples...")

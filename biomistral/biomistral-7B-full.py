# biomistral_full_ft.py
# ==================== BIOMISTRAL-7B-INSTRUCT - FULL FINETUNE (1x GPU, NO LoRA/NO QUANT) ====================
...
# Optimisé pour RTX 6000 Ada (48GB VRAM) - Biomistral 7B (plus léger que Qwen)
MAX_SEQ_LEN  = int(os.getenv("MAX_SEQ_LEN", 4096))    # Gardé à 4096
NUM_EPOCHS   = int(os.getenv("NUM_EPOCHS", 5))
BATCH_SIZE   = int(os.getenv("BATCH_SIZE", 2))        # 2 per-device (Biomistral économe en VRAM)
GRAD_ACCUM   = int(os.getenv("GRAD_ACCUM", 4))        # 4 (effective BS = 2*4 = 8)
LR           = float(os.getenv("LR", "2e-5"))         # 2e-5 (Biomistral moins sensible que Qwen)
...
MODEL_DIR    = Path(__file__).parent / "model"  # Sauvegarde dans biomistral/model
